...
The short name identifier for Passenger Services used across MDS is `delivery`.
...
Events require a valid `trip_id` in events where `event_types` contains `reservation_start`, `reservation_stop`, `trip_start`, `trip_stop`, `trip_end`, `client_cancellation`, `provider_cancellation`, or `driver_cancellation`. For the robots, the notion of driver does not exist. 
...
The `trip_attributes` array is used with in delivery.
...
| `reserved`                   | `unknown`         | N/A          | `comms_lost`             | The vehicle went out of comms while being reserved by a customer                                                |
